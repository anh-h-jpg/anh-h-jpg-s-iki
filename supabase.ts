...
// Clean up any legacy localStorage credentials if left over from previous versions
if (typeof window !== 'undefined') {
  try {
    localStorage.removeItem('app_lunch_supabase_url');
    localStorage.removeItem('app_lunch_supabase_anon_key');
  } catch {
    // ignore
  }
}
...
class LocalLunchEngine {
  ...
  public markPaymentPending(userId: string, weeklyMenuId: string, amountDue: number): Payment {
    let p = this.payments.find((item) => item.user_id === userId && item.weekly_menu_id === weeklyMenuId);
    if (p) {
      p.status = 'pending';
      p.method = 'transfer';
      if (!p.amount_due || p.amount_due <= 0) {
        p.amount_due = amountDue;
      }
    } else {
      p = {
        id: `pay-${Date.now()}-${userId.slice(0, 4)}`,
        user_id: userId,
        weekly_menu_id: weeklyMenuId,
        amount_due: amountDue,
        status: 'pending',
  ...
  public rejectPayment(paymentId: string): boolean {
    const p = this.payments.find((item) => item.id === paymentId);
    if (p) {
      p.status = 'unpaid';
      this.saveToStorage();
  ...
  public toggleMenuItemClosed(id: string, closed: boolean) {
